...
AddonParser::addAddon( 'sp_vs_best_selling', 'sp_vs_best_selling_addon' );
AddonParser::addAddon( 'sp_vs_best_selling_item', 'sp_vs_best_selling_item_addon' );
AddonParser::addAddon('sp_vs_best_selling_content', 'sp_vs_best_selling_content_addon');
function sp_vs_best_selling_addon( $atts, $content ) {
	$class = $title = '';
	...
	ob_start();
	?>
	<div class="list_best_selling vs_clear tabs">
		<h2><i class="zmdi zmdi-badge-check zmdi-hc-fw"></i> <?php echo $title;?></h2>
		<ul class="list_label tab">
			<?php echo AddonParser::spDoAddon($content);?>
		</ul>
		<div class="content_tab">
			<?php
			$tab_content = str_replace('sp_vs_best_selling_item', 'sp_vs_best_selling_content', $content);
...
function sp_vs_best_selling_item_addon( $atts ){
	$cat = $title = $icon = '';
	extract(spAddonAtts(array(
		"cat"=>'',
		'title' => '',
...
function sp_vs_best_selling_content_addon($atts)
{
	$cat = $title = $icon = '';
	extract(spAddonAtts(array(
		"cat"=>'',
	...
	$products = $productModel->getProductListing('topten', 10, true, true, false, true, $cat);
	?>
