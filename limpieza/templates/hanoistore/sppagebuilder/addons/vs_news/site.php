...
AddonParser::addAddon( 'sp_vs_news', 'sp_vs_news_addon' );
AddonParser::addAddon( 'sp_vs_news_item', 'sp_vs_news_item_addon' );
AddonParser::addAddon('sp_vs_news_content', 'sp_vs_news_content_addon');
function sp_vs_news_addon( $atts, $content ) {
	$class = $title = '';
	...
	<div class="list_featured vs_clear tabs">
		...
		<div class="content_tab">
			<?php
			$tab_content = str_replace('sp_vs_news_item', 'sp_vs_news_content', $content);
...
function sp_vs_news_item_addon( $atts ){
	$cat = $title = $icon = '';
	extract(spAddonAtts(array(
		"cat"=>'',
		'title' => '',
...
function sp_vs_news_content_addon($atts)
{
	...
	if($cat=='all') {
		$products = $productModel->getProductListing('latest', $number, true, true, false, false);
	} else {
