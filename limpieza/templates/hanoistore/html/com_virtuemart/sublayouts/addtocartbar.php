...
if (!VmConfig::get('use_as_catalog', 0)  ) { ?>
	...
	if (($stockhandle == 'disableit' or $stockhandle == 'disableadd') and ($product->product_in_stock - $product->product_ordered) < 1) { ?>
		...
		if (!( VmConfig::get('askprice', true) and empty($tmpPrice) ) ) { ?>
			<?php if ($product->orderable && $show_qty!=false) { ?>
			...
			if($show_qty==false) { ?>
				<span class="addtocart-button">
					<button type="submit" name="addtocart" class="addtocart-button" value="Add to Cart" title="Add to Cart"><i class="zmdi zmdi-plus zmdi-hc-fw"></i> <?php echo vmText::_( 'COM_VIRTUEMART_CART_ADD_TO' );?></button>
				</span>
